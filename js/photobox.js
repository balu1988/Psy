;(function($, doc, win){
    ...
    var Photobox, photobox, options, images=[], imageLinks, activeImage = -1, activeURL, lastActive, activeType, prevImage, nextImage, thumbsStripe, docElm, APControl, changeImage,
        ...
        defaults = {
            ...
            captionTmpl   : '<div class="counter">{currentImageIdx}/{totalImagesCount}</div>',
    ...
    function onSwipe(e, Dx, Dy){
        if( Dx == 1 ){
            image.css({transform:'translateX(25%)', transition:'.1s', opacity:0});
            setTimeout(function(){ changeImage(prevImage) }, 20);
        }
        else if( Dx == -1 ){
            image.css({transform:'translateX(-25%)', transition:'.1s', opacity:0});
            setTimeout(function(){ changeImage(nextImage) }, 20);
        }
